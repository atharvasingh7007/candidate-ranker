...
* **Two-Stage Semantic Matching:** We first use `sentence-transformers/all-MiniLM-L6-v2` as a Bi-Encoder to quickly score all 100K candidates. Then, we extract the Top 5,000 and run them through a powerful **Cross-Encoder** (`ms-marco-MiniLM-L-6-v2`) which simultaneously processes the JD and candidate text with deep cross-attention, yielding vastly superior semantic ranking.
* **Hyperparameter-Tuned Learning-to-Rank (LTR):** We generate preference pairs based on extracted features and train an **XGBoost** model. Before training the final tree, we run a `RandomizedSearchCV` to dynamically find the mathematically optimal `max_depth`, `learning_rate`, and `subsample` parameters for our data. XGBoost was chosen because it natively handles non-linear feature interactions much better than simple linear weights, and inference takes literally milliseconds.
